...
DEFAULT_STATE_JSON='{"last_fired":{},"session_tool_count":0,"session_start_time":"","write_tool_count":0,"error_count":0,"nudge_streak":{"id":"","count":0}}'
...
state_action_jq() {
  ...
  case "$action" in
    ...
    get_write_count)
      ...
      ;;
    get_error_count)
      jq -r '.error_count // 0' "$STATE_FILE" 2>/dev/null || printf '0\n'
      ;;
    get_streak)
...
def default_state():
    return {"last_fired": {}, "session_tool_count": 0, "session_start_time": "", "write_tool_count": 0, "error_count": 0, "nudge_streak": {"id": "", "count": 0}}
...
elif action == "get_error_count":
    print(int(data.get("error_count", 0)))
...
evaluate_condition() {
  ...
  case "$condition" in
    ...
    "last_output_has_error == true")
      ...
      ;;
    "write_tool_count >= "*)
      local threshold="${condition#write_tool_count >= }"
      if [[ "$WRITE_TOOL_COUNT" =~ ^[0-9]+$ && "$threshold" =~ ^[0-9]+$ && "$WRITE_TOOL_COUNT" -ge "$threshold" ]]; then
        printf 'true\n'
      else
        printf 'false\n'
      fi
      ;;
    "error_count >= "*)
      local threshold="${condition#error_count >= }"
      if [[ "$ERROR_COUNT" =~ ^[0-9]+$ && "$threshold" =~ ^[0-9]+$ && "$ERROR_COUNT" -ge "$threshold" ]]; then
        printf 'true\n'
      else
        printf 'false\n'
      fi
      ;;
    "session_tool_count <= "*)
      local threshold="${condition#session_tool_count <= }"
      if [[ "$TOOL_CALL_COUNT" =~ ^[0-9]+$ && "$threshold" =~ ^[0-9]+$ && "$TOOL_CALL_COUNT" -le "$threshold" ]]; then
        printf 'true\n'
      else
        printf 'false\n'
      fi
...
ERROR_COUNT="$(state_action get_error_count 2>/dev/null || printf '0\n')"
